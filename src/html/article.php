  ...
  <nav class="navbar navbar-expand" id="navbar-kedua">
    ...
    <main>
        <div class="container mt-5">
            <div class="row">
                <?php
                // Perulangan untuk menampilkan setiap artikel
                if ($result->num_rows > 0) {
                    while($row = $result->fetch_assoc()) {
                        $isi_artikel = strlen($row['isi']) > 500 ? substr($row['isi'], 0, 500) . "..." : $row['isi'];
                        echo '<div class="col-md-12 mb-4">
                                <div class="card">
                                    <div class="row no-gutters">
                                        <div class="col-md-4">
                                            <img src="../uploads-artikel/' . $row['gambar'] . '" class="card-img" alt="Gambar Artikel">
                                        </div>
                                        <div class="col-md-8">
                                            <div class="card-body" style="width: 90%; margin-left: 50px;">
                                                <h5 class="card-title">' . $row['judul'] . '</h5>
                                                <p class="card-text">
                                                    <small class="text-muted">Dibuat oleh <strong>' . $row['penulis'] . '</strong> pada <strong>' . $row['tanggal'] . '</strong></small>
                                                </p>
                                                <p class="card-text">' . $isi_artikel . '</p>
                                                <a href="detail-artikel.php?id=' . $row['id_artikel'] . '" class="btn btn-primary">Baca Selengkapnya</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
